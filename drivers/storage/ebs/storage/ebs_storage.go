	...
	for _, volume := range ec2vols {
		...
		for _, attachment := range volume.Attachments {
			...
			attachmentSD := &types.VolumeAttachment{
				...
				InstanceID: &types.InstanceID{ID: *attachment.InstanceId, Driver: ebs.Name},
...
func (d *driver) GetVolumeMapping() ([]*types.VolumeDevice, error) {
	...
	for _, blockDevice := range blockDevices {
		sdBlockDevice := &types.VolumeDevice{
			ProviderName: d.Name(),
			InstanceID:   &types.InstanceID{ID: d.instanceDocument.InstanceID, Driver: ebs.Name},
			Region:       d.instanceDocument.Region,
...
func (d *driver) maxRetries() int {
	...
	if maxRetriesString := d.config.GetString("ebs.maxRetries"); maxRetriesString != "0" {
		maxRetries := d.config.GetInt("ebs.maxRetries")
		if maxRetries <= 0 {
